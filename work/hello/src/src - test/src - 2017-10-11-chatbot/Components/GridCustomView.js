  ...
  handleKeyDown (e) {
    ...
    if (e.key === 'ArrowDown') {
      if (nextIndex === -1) {
        ...
        return this
      } else if (nextIndex === this.NUMBER_OF_ITEMS || (nextIndex === this.NUMBER_OF_ITEMS - 1)) {
      ...
      //  document.getElementById('react-tabs-4').focus()
        var elementsList = document.querySelector('.react-tabs__tab--selected')
        document.getElementById(elementsList.id).focus()
        domElems.navPage.children[nextIndex].className = 'unselected'
        nextIndex = -1
      } else if (nextIndex === 1) {
        //document.getElementById('react-tabs-4').focus()
