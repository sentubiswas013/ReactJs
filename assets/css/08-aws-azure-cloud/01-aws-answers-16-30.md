...
| Component | Single AZ (Low Availability) | Multi-AZ (High Availability) |
|-----------|------------------------------|------------------------------|
| **Compute** | 1 EC2 instance | Auto Scaling across 3 AZs |
| **Load Balancer** | None | Application Load Balancer |
| **Database** | Single RDS instance | Multi-AZ RDS + Read Replicas |
| **Storage** | EBS (single AZ) | S3 (replicated across AZs) |
| **DNS** | Single IP | Route 53 with health checks |
| **Monitoring** | Basic | CloudWatch alarms + dashboards |
| **Downtime Risk** | High (single point of failure) | Low (automatic failover) |
