...
We optimize a slow React page using memoization (React.memo, useMemo, useCallback), code splitting (lazy loading), virtualization for large lists, optimizing images, and reducing unnecessary re-renders.

**Main Optimization Techniques (One line each)**
...
* **Avoid Re-renders** – Use keys properly and avoid inline functions.
* **Bundle Optimization** – Remove unused code and compress files.
* **Profiler** – Use React DevTools Profiler to find slow components.
...
Class components are migrated to hooks by converting state to useState, lifecycle methods to useEffect, class methods to functions, and refs to useRef. Complex logic can be moved into custom hooks.
...
| this.refs            | useRef                        |
| Class methods        | Normal function / useCallback |

---

**Migration Steps (Short)**

1. Convert **this.state → useState**
2. Convert **componentDidMount → useEffect**
3. Convert **componentDidUpdate → useEffect with dependency**
4. Convert **componentWillUnmount → useEffect cleanup**
5. Convert **class methods → normal functions**
6. Convert **refs → useRef**
7. Move reusable logic → **custom hooks**
