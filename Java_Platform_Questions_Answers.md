...
* Java is popular because it’s **platform-independent**—write once, run anywhere.
...
👉 Same program runs on Windows, Linux, or Mac.

---

### 2. What is platform independence?

* Platform independence means **Java code doesn’t depend on the OS**.
...
👉 JVM acts as a bridge between Java code and OS.
...
👉 This is the main reason Java is portable.
...
* **JDK** → JRE + development tools (to write Java programs).
...
👉 Developers need **JDK**, users need only **JRE**.
...
* Java doesn’t support **pointers**, making it safer.
...
👉 Java is safer and easier for large applications.
...
👉 Classes are loaded **only when needed**, improving performance.
...
* Examples: `int → Integer`, `double → Double`.
...
* Yes, it’s called a **local inner class**.
...
* No index handling → **cleaner and less error-prone**.
...
* `HashSet` → basic implementation, no order.
...
* `SortedSet` → no duplicates **and elements are sorted** automatically.
...
* `PriorityQueue` → queue with natural or custom ordering.
...
* `PriorityBlockingQueue` → priority-based thread-safe queue.
...
* `SortedMap` → maintains keys in **natural sorted order**.
...
* Examples:
  ...
  * `BiPredicate<T, U>` → returns boolean, 2 args.
  * `BiFunction<T, U, R>` → 2 args, returns result.
  * `BiConsumer<T, U>` → 2 args, returns nothing.
...
* **Autoboxing/unboxing** → auto conversion between primitives and wrappers.
* **Enums** → type-safe constants.
* **Varargs** → variable-length arguments.
* **Metadata/Annotations** → `@Override`, `@Deprecated`.
...
* **Scripting support** → integrate JavaScript via `javax.script`.
* **JDBC 4.0** → improved database connectivity.
* **Compiler API** → programmatically compile code.
...
* **Strings in switch** → switch-case works with strings.
* **Try-with-resources** → auto-close resources.
* **Numeric literals with underscores** → improves readability.
* **Multi-catch exceptions** → catch multiple exceptions in one block.
...
* **Lambda expressions** → concise functions.
